...
static void		suffix_double_hex(t_printf *pf, int exponent)
{
	unsigned int	exp;
	char			suffix[3];
	char			*str_exp;
...
static void		prefix_double_hex(t_printf *pf, t_double *t_f)
{
	char	prefix[4];
	size_t	i;
...
static void		write_double_hex(t_printf *pf, t_double *t_f)
{
	if (t_f->is_nan || t_f->is_inf)
		return (print_inf_nan(pf, t_f));
	if (!(pf->flags & PREC_FLAG))
		pf->precision = 13;
	pf->str = pf_mantissa_to_hex(t_f, pf->precision);
	if (pf->str)
		suffix_double_hex(pf, t_f->real_exponent);
	if (pf->str)
		prefix_double_hex(pf, t_f);
	if (pf->str)
		pf_pad_field_width(pf);
	if (pf->str)
	{

...
static void		suffix_ldouble(t_printf *pf)
{
	if (pf->flags & HASH_FLAG && pf->precision == 0)
		pf->str = ft_strsuffix(pf->str, ".", 1);
}

static void		prefix_ldouble(t_printf *pf, t_double *t_f)
{
	char	prefix[2];
...
static void		write_ldouble(t_printf *pf, t_double *t_f)
{
	long double	f;
	...
	pf->str = ft_ldtoa(f, pf->precision);
	if (pf->str)
		suffix_ldouble(pf);
	if (pf->str)
		prefix_ldouble(pf, t_f);
	if (pf->str)
		pf_pad_field_width(pf);
	if (pf->str)
